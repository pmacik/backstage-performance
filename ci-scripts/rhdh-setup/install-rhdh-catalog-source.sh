...
ENABLE_CACHE_LABEL_FILTER="${ENABLE_CACHE_LABEL_FILTER:-false}"
...
# TMPDIR=$(mktemp -d)
TMPDIR=$(python3 -c 'import os, sys; print(os.path.realpath(sys.argv[1]))' "${TMP_DIR:-.tmp}")
...
spec:
  ...
  config:
    ...
    - name: ENABLE_CACHE_LABEL_FILTER
      value: \"${ENABLE_CACHE_LABEL_FILTER}\"
" >"$subscription"
