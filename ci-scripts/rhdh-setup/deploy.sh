...
backstage_install() {
    ...
    #if [ "${AUTH_PROVIDER}" == "keycloak" ]; then yq -i '. |= . + {"signInPage":"auto"}' "$TMP_DIR/app-config.yaml"; fi
    #if [ "${AUTH_PROVIDER}" == "keycloak" ]; then yq -i '.auth.providers |= . + {"oauth2Proxy":{}}' "$TMP_DIR/app-config.yaml"; else yq -i '.auth.providers |= . + {"guest":{"dangerouslyAllowOutsideDevelopment":true}}' "$TMP_DIR/app-config.yaml"; fi
    if [ "$INSTALL_METHOD" == "helm" ]; then
